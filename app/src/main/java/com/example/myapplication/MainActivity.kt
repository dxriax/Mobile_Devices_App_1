...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Log.d("main activity", "on create exexute")

        etNumOptions = findViewById<EditText>(R.id.etNumOptions) as EditText
        etVotingOptions = findViewById<EditText>(R.id.etVotingOptions) as EditText
        btnAddVote = findViewById<Button>(R.id.btnAddVote) as Button


        // Set OnClickListener for btnAddVote
        btnAddVote?.setOnClickListener {
            onClick(it)
        }

        // Set onFocusChangeListener for etNumOptions
    ...
    fun onClick(v: View) {
        ...
        if (v == btnAddVote) {
            var numOptions = etNumOptions!!.text.toString().toIntOrNull() ?: 3
            var trimmedStringsOption =
                etVotingOptions!!.text.toString().split(",").map { it.trim() }.toMutableList()

            // String adjustment - size of the list will be equal to number of options entered
            trimmedStringsOption =
                adjustOptions(trimmedStringsOption, numOptions) // 2. fun adjustOptions
